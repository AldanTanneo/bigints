...
is
   ...
   is
      ...
      function Trailing_Zeros_64 (Value : U64) return Natural with
   ...
   function Leading_Zeros (Value : U32) return Natural is
     (Builtins.Leading_Zeros_32 (Value)) with
     Pre  => Value > 0,
     Post =>
   ...
   function Trailing_Zeros (Value : U32) return Natural is
     (Builtins.Trailing_Zeros_32 (Value)) with
     Pre  => Value > 0,
     Post =>
      Trailing_Zeros'Result < 32
      and then (Value / 2**Trailing_Zeros'Result) * 2**Trailing_Zeros'Result =
        Value
      and then Value / 2**(Trailing_Zeros'Result) mod 2 = 1,
     Inline_Always;

   function Shift_Left (Value : U32; Amount : Natural) return U32 with
     Pre    => Amount < 32, Post => Shift_Left'Result = Value * 2**Amount,
     Global => null, Import, Convention => Intrinsic, Static, Inline_Always;

   function Shift_Right (Value : U32; Amount : Natural) return U32 with
     Pre    => Amount < 32, Post => Shift_Right'Result = Value / 2**Amount,
     Global => null, Import, Convention => Intrinsic, Static, Inline_Always;

   function Leading_Zeros (Value : U64) return Natural is
     (Builtins.Leading_Zeros_64 (Value)) with
     Pre  => Value > 0,
     Post =>
   ...
   function Trailing_Zeros (Value : U64) return Natural is
     (Builtins.Trailing_Zeros_64 (Value)) with
     Pre  => Value > 0,
     Post =>
   ...
   function Shift_Left (Value : U64; Amount : Natural) return U64 with
     Pre    => Amount < 64, Post => Shift_Left'Result = Value * 2**Amount,
     Global => null, Import, Convention => Intrinsic, Static, Inline_Always;

   function Shift_Right (Value : U64; Amount : Natural) return U64 with
     Pre    => Amount < 64, Post => Shift_Right'Result = Value / 2**Amount,
     Global => null, Import, Convention => Intrinsic, Static, Inline_Always;

   function Shift_Left (Value : U128; Amount : Natural) return U128 with
     Pre    => Amount < 128, Post => Shift_Left'Result = Value * 2**Amount,
     Global => null, Import, Convention => Intrinsic, Static, Inline_Always;

...
is
   ...
   function Is_Hex (C : Character) return Boolean is
     ((C in '0' .. '9' or else C in 'a' .. 'f' or else C in 'A' .. 'F')) with
     Ghost;

   function From_Hex (Value : String) return Uint with
     Pre =>
      (Value'Length > 0 and then Value'Length <= 16 * N
       and then
   ...
   function Add_Carry (A, B : Uint; Carry : U64) return Uint_Carry with
     Post => Add_Carry'Result.Carry in 0 .. 2;
   --  Computes `A + B + Carry`, returning the result along with the new carry

   function Sub_Borrow (A, B : Uint; Borrow : U64) return Uint_Carry with
   ...
   function Div_Rem_Limb (Value : Uint; Rhs : U64) return Quotient_Rem with
     Pre => Rhs /= 0;
   function Rem_Limb (Value : Uint; Rhs : U64) return U64 with
     Pre => Rhs /= 0;

   procedure Impl_Div_Rem
     (Lhs, Rhs : Uint; Quotient, Remainder : out Uint)
     --  Implement constant time division
     with
     Pre => (for some I in 1 .. N => Rhs (I) /= 0);

   function "not" (A : Uint) return Uint with
     Inline, Post => (for all I in 1 .. N => "not"'Result (I) = not A (I));
   --  Compute the bitwise NOT, ~A

   function "and" (A, B : Uint) return Uint with
     Inline,
     Post => (for all I in 1 .. N => "and"'Result (I) = (A (I) and B (I)));
   --  Compute the bitwise AND, A & B

   function "and" (A : Uint; B : U64) return Uint with
     Inline, Post => (for all I in 1 .. N => "and"'Result (I) = (A (I) and B));
   --  Compute the bitwise AND of every limb of A with B

   function "or" (A, B : Uint) return Uint with
     Inline,
     Post => (for all I in 1 .. N => "or"'Result (I) = (A (I) or B (I)));
   --  Compute the bitwise OR, A | B

   function "xor" (A, B : Uint) return Uint with
     Inline,
   ...
   function "/" (A, B : Uint) return Uint with
     Pre => (for some I in 1 .. N => B (I) /= 0);
   --  Compute A / B for B != 0

   function "mod" (A, B : Uint) return Uint with
     Pre => (for some I in 1 .. N => B (I) /= 0);
   --  Compute A % B for B != 0
   ...
   function Bit_Vartime (Value : Uint; Amount : Natural) return Boolean with
     Pre => Amount < BITS;
   ...
   procedure CSwap (A, B : in out Uint; C : Const_Choice.Choice) with
     Post =>
      (if Const_Choice.To_Bool (C) then
   ...
   function Cond_Select (A, B : Uint; C : Const_Choice.Choice) return Uint with
     Post =>
      (for all I in 1 .. N =>
   ...
   function Shl_Vartime (Value : Uint; Amount : Natural) return Uint with
     Pre => Amount <= BITS;
   --  Shift integer left by `Amount` bits in variable time with Amount

   function Shl (Value : Uint; Amount : Natural) return Uint with
     Pre => Amount <= BITS;
   --  Shift integer left by `Amount` bits

   function Shl_Limb (Value : Uint; Shift : Natural) return Uint_Carry with
     Pre => Shift < 64;
   --  Shift integer left by `Amount` bits, where `Amount` fits in a single
   --  limb. Return the shifted integer and a carry

   function Shl1 (Value : Uint) return Uint_Carry with
     Post => Shl1'Result.Carry in 0 .. 1;
   --  Shift integer left by one bit. Return the shifted integer and a carry

   function Shl1 (Value : Uint) return Uint is (Uint_Carry'(Shl1 (Value)).Res);
   --  Shift integer left by one bit.

   function Shr_Vartime (Value : Uint; Amount : Natural) return Uint with
     Pre => Amount <= BITS;
   --  Shift integer right by `Amount` bits in variable time with Amount

   function Shr (Value : Uint; Amount : Natural) return Uint with
     Pre => Amount <= BITS;
   --  Shift integer right by `Amount` bits

   function Shr_Limb (Value : Uint; Shift : Natural) return Uint_Carry with
     Pre => Shift < 64;
   --  Shift integer right by `Amount` bits, where `Amount` fits in a single
   --  limb. Return the shifted integer and a carry

   function Shr1 (Value : Uint) return Uint_Carry with
     Post => Shr1'Result.Carry in 0 .. 1;
   --  Shift integer right by one bit. Return the shifted integer and a carry

   function Shr1 (Value : Uint) return Uint is (Uint_Carry'(Shr1 (Value)).Res);
   --  Shift integer right by one bit

   function Leading_Zeros (Value : Uint) return Natural with
     Post =>
      Leading_Zeros'Result <= BITS
      and then
      (for all I in N - Leading_Zeros'Result / 64 + 1 .. N => Value (I) = 0)
      and then
      (Leading_Zeros'Result = BITS

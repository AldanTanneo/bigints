...
package Bigints.Uints with SPARK_Mode => On, Pure, Initial_Condition => (BITS mod 64 = 0) is
   ...
   with
     Pre =>
       (Value'Length > 0
        ...
        and then (for all I in Value'Range => Value (I) = '_' or else Is_Hex (Value (I))));
   --  Parse a big-endian (most significant digit first) hexadecimal string
   --  into a Uint

   function From_U64 (X : U64) return Uint
   is ([X, others => 0])
   with Inline;
   ...
   with Inline, Post => (for all I in 1 .. N => A (I) = Truncate'Result (I));
   --  Return the lower limbs as a Uint

   function Truncate_Upper (A : Wide_Uint) return Uint
   with Inline, Post => (for all I in 1 .. N => A (N + I) = Truncate_Upper'Result (I));
   ...
   with
     ...
     Post => Sub_Borrow'Result.Carry = 0 or else Sub_Borrow'Result.Carry = U64'Last;
   ...
   function Div_Rem_Limb_With_Reciprocal (U : Uint; Re : Primitives.Recip) return Quotient_Rem;
   function Rem_Limb_With_Reciprocal (U : Uint; Re : Primitives.Recip) return U64;
   function Rem_Limb_With_Reciprocal_Wide (Lo, Hi : Uint; Re : Primitives.Recip) return U64;
   ...
   with Inline, Post => (for all I in 1 .. N => "and"'Result (I) = (A (I) and B (I)));
   --  Compute the bitwise AND, A & B

   function "and" (A : Uint; B : U64) return Uint
   with Inline, Post => (for all I in 1 .. N => "and"'Result (I) = (A (I) and B));
   --  Compute the bitwise AND of every limb of A with B

   function "or" (A, B : Uint) return Uint
   with Inline, Post => (for all I in 1 .. N => "or"'Result (I) = (A (I) or B (I)));
   --  Compute the bitwise OR, A | B

   function "xor" (A, B : Uint) return Uint
   with Inline, Post => (for all I in 1 .. N => "xor"'Result (I) = (A (I) xor B (I)));
   ...
   function Bit_Vartime (Value : Uint; Amount : Natural) return Const_Choice.Choice
   with Pre => Amount < BITS;
   --  Variable time relative to Amount
   ...
   function "=" (A, B : Uint) return Const_Choice.Choice
   with Inline, Post => Const_Choice.To_Bool ("="'Result) = (for all I in 1 .. N => A (I) = B (I));
   function "<=" (A, B : Uint) return Const_Choice.Choice
   with Inline;
   ...
   with
     Post =>
       (if Const_Choice.To_Bool (C)
        then (for all I in 1 .. N => B (I) = A'Old (I) and then A (I) = B'Old (I))
        else (for all I in 1 .. N => A (I) = A'Old (I) and then B (I) = B'Old (I)));
   --  Constant time swap
   ...
   with
     Post =>
       ...
       and then (for all I in N - Leading_Zeros'Result / 64 + 1 .. N => Value (I) = 0)
       and then (Leading_Zeros'Result = BITS or else Value (N - Leading_Zeros'Result / 64) /= 0);

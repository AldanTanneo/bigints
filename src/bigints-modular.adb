...
is
   ...
   overriding
   function "+" (A, B : Fp) return Fp is
   begin
      return Fp (Uints_Modulo.Add_Mod (Uint (A), Uint (B), P));
   end "+";

   overriding
   function "-" (A : Fp) return Fp is
   begin
   ...
   overriding
   function Square (A : Fp) return Fp is
   ...
   function "**" (A : Fp; N : Uint) return Fp
   is (Pow (A, N));
   ...
   is (Pow (A, P_MINUS_TWO));

   function Inv_Vartime (A : Fp) return Fp
   is (Pow_Vartime (A, P_MINUS_TWO));

   overriding
   function Cond_Select (A, B : Fp; C : Const_Choice.Choice) return Fp is
   begin
      return Fp (Cond_Select (Uint (A), Uint (B), C));
   end Cond_Select;

   overriding
   procedure CSwap (A, B : in out Fp; C : Const_Choice.Choice) is
   begin

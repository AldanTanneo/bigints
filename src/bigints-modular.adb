...
is
   ...
   begin
      for I in 1 .. N loop
         ...
         LimbCarry := Mac (Value (I), U, Modulus (I), 0);

         for J in 1 .. N loop
            LimbCarry := Mac (Value (I + J), U, Modulus (J), Carry);
            Value (I + J) := New_Limb;
         end loop;

         SumCarry := Add_Carry (Value (N + I), Carry, Meta_Carry);
         Value (N + I) := New_Sum;
      end loop;
   ...
   function Len (X : Uint) return Natural
   with
     Inline,
     Post =>
       Len'Result <= BITS
   ...
   function Inv (Y : Fp) return Fp is
      ...
      function Mul_Add_Shift
        (A : Uint; F : U64; B : Uint; G : U64; K : Natural) return Uint
      with Pre => K in 1 .. 64
      is
